...
void Client::loopEvent(){
    ...
    try {
        ...
        debug("failed to recv_data on client %d with error=%s code=%d",_fd,e.what(),e.code());
        ...
        case MESSAGE_PLIST:
        {
            ...
            if (message == "Listen") {
                ...
                p_rsp = plist_new_dict();
                {
                    ...
                    cleanup([&]{
                        safeFreeCustom(plistbin, plist_mem_free);
                    });
                ...
                plist_from_memory(pairRecord, (uint32_t)pairRecord_len, &p_parsedPairRecord, nullptr);
